...
def add_fingerprint_to_db(file, db_name=DB, collection_name=COLLECTION, track_id=None, sample_rate= SAMPLE_RATE, footprint=FOOTPRINT_SIZE, fps=FRAMES_PER_SECOND, t_start=TARGET_START, t_height=TARGET_HEIGHT, t_width=TARGET_WIDTH, peak_threshold=PEAK_THRESHOLD):
    ...
    fingerprint = generate_fingerprint(file, track_id, sample_rate=sample_rate, footprint=footprint, fps=fps, t_start=t_start, t_height=t_height, t_width=t_width, peak_threshold=peak_threshold)
...
def match_fingerprint(fingerprint, collection):
    ...
    # initialise dictionary to store largest bin of time_ds as percentage for each db track
    match_results = {}
    for key in time_ds:
        match_results[key] = 0
