...
def generate_fingerprint(file, track_id=None, footprint=FOOTPRINT_SIZE, fps=FRAMES_PER_SECOND, target_start=TARGET_START, target_height=TARGET_HEIGHT, target_width=TARGET_WIDTH, peak_threshold=PEAK_THRESHOLD):
    spectrogram = generate_spectrogram(file, fps)
    peaks, peaks_x, peaks_y = generate_starmap(spectrogram, footprint, peak_threshold)
